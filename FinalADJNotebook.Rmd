...
**As noted later in the file and something I only now realize while reviewing this a few years later (I had not bothered to look again since I last turned this in because of other Academics taking priority) is that many gameids seem to be missing! Things like "ESPORTSTMNT04_1880348" are missing, and the only ones that are showing up are all numerical only. Whereas anything that actually includes a character is not on any of those 100 pages! And these gameids are plainly present within CompleteData, a dataset made later on. I can't help but wonder if some way in which the gameid values are inputted (since they differ in format) has affected how R is able to read such a value, but on the other hand, I can see them constrained to the box in the data so I really don't know what is going on. Even in later data sets these missing groups just suddenly appear, and I see no difference as to why. Is the data set too large to include all of them?
...
data %>%
  ...
  count()
```
Even after setting the data to be only games that had complete data thereby limiting the total to about half is that the dataset is still too large (at least for RStudio, after knitting, I can see that this table goes to the full 600+ pages)! This limited table view only goes up to about a hundred pages, therefore, I have another thing I want to try.

```{r}
CompleteGamesGameidCount <- data %>% #btw this is going to look exactly as it sounds. I'm attempting what I tried before just with putting it all in a table rather than relying on an in-line preview
  filter(datacompleteness == "complete") %>%
  group_by(gameid) %>%
  count()
#now, hypothetically, this should produce for me a new table which I can examine that will hopefully include every duplicate GameID. I know from later analysis that there is at least one game that has about 14 duplicates! So I should be able to find a gameid that when grouped in this table has a count of about 192 ((14 duplicates+ 1 original)*12 (for each corresponding row associated with each gameid as explained above)! Why did this not show up when I verified it in the next data block as I organized the count by descending order? I do not know, but here we go!

CompleteGamesGameidCount #added so can be seen on HTML
  
CompleteGamesGameidCount %>%
  filter(n != 12) #going to try and use this to filter out games where the expected 12 is 12 so I can see only the abnormalities.
```
In this table we do see that the data is not as clean as it may have been once supposed to be given previous analysis and the analysis done in this following block. Somehow, the arrange does not seem to be working properly either. In any case, there are actually TWO gameids with counts of 192 (ESPORTSTMNT04_1880348 and ESPORTSTMNT04_1880351) and even in the complete data here, there are still games (48 count which means 4 games) with gameids listed as NA which does not sound complete to me.

```{r}
CleanCompleteGamesGameidCount <- data %>%
  filter((datacompleteness == "complete") & (!is.na(gameid)) & (!duplicated(gameid))) %>% #looking for complete data where the gameid is not NA and where the gameid is not duplicated, should return 1 for count for everything.
  group_by(gameid) %>%
  count() %>%
  arrange(desc(n))

CleanCompleteGamesGameidCount #added so you can see
```

duplicated is not a boolean we learned about in 2021, and I wish I had, it is a good way to get rid of these duplicates. I'm going to attempt applying this to the CompleteData tables.
...
**My old code was for some reason wrong. I had been taught to use arrange(desc(n())) like that, but according to online forums, I only need to put arrange(desc(n)), I actually distincly remember being taught that and it working before, but perhaps it is simply a Mandela effect. Either way, it was NOT verified in 2021, but is now that there are duplicates!
...
CompleteData #added to be seen

CleanCompleteData <- unique(CompleteData) %>%
  filter(!is.na(gameid))#coming back to this from later because there are (as mentioned above) some games that are included a whopping 15 times. I'm hoping that this function removes duplicated rows where all the data is the same and not just based on gameid. I am also removing any NA gameids since I operate on the assumption later that all gameids in complete data had gameids.

CleanCompleteData #added to be seen
  
```
This represents 93,648/133,428 rows or about 61% of the data is complete, another 9% is partial (not shown) #I don't know where these numbers came from. The data has 133,428 objects yes, but CompleteData (even back then) seems to have only reported 81,516 objects, not 93,648. So this is another error, how on earth did I get an A on this??

It seems to have worked, and somehow those numbers don't exist. With CleanCompleteData, the total is 81,144 objects which I think makes sense because when you remove the 48 lines from gameids being NA (even in the complete data!) and the two sets of 180 lines (totaling to 360) well I am honestly confused that doesn't math out. It would seem to me that there are still about 36 more lines that need to be removed. Let's just check again if there are any duplicate games.

```{r}
CleanCompleteGameidCount <- CleanCompleteData %>%
  group_by(gameid) %>%
  count() %>%
  arrange(desc(n))

CleanCompleteGameidCount #added to be seen
```
Yes... intriguing, even after things have been made unique with CleanCompleteData there is still an issue with the gameid ESPORTSTMNT04_1880348 as it seems to have exactly 48 references to it when it should only have 12, accounting for our 36 lines that are yet to be removed.

I'm going to toy with this because this is data I'm familiar with and am continuing to learn with it.

```{r}
CleanCompleteData2 <- CleanCompleteData %>%
  distinct(gameid, playerid, .keep_all = TRUE) #what this is doing is it is targeting those 48 lines that are duplicated by saying we want to keep every line that has a distinct gameid and playerid because each pair of those is a completely unique combo: gameid and 1-10 (and 100 and 200 for the teams) thus eleminating any other duplicated lines. I'm interested to see how this affects the rest of the data. 

CleanCompleteData2 #added to be seen
```
According to math, that maths out as CleanCompleteData2 has successfully removed all duplicated rows coming to a total of 81,108 out of the original 133,428 rows.
...
```{r}
CompleteTurretPlates <- data %>% # as a later update, I'm moving this into its own dataframe rather than relying on a tibble since this is larger than 1000 rows. 
  group_by(gameid) %>%
  filter(!is.na(turretplates)) %>%
  select(gameid, datacompleteness, turretplates) %>%
  arrange(desc(turretplates))

CompleteTurretPlates #added to be seen
```
...
```{r}
CleanCompleteData2 %>% #As of November 9, All following references to CompleteData are being updated to CleanCompleteData2
  filter((playerid == "100" | playerid == "200") #this calls for the teams
         & (gameid == "ESPORTSTMNT03_2074070" | gameid == "ESPORTSTMNT06_1881146" | gameid == "ESPORTSTMNT02_2151778")) %>%
...
```{r}
PlayerWRs <- CleanCompleteData2 %>% #also updating this to be its own dataframe because it is over 1000 lines
  group_by(player, league) %>%
  summarize(WR = sum(result)/n(), sum(result), totalG = n()) %>%
  arrange(desc(WR), desc(totalG))

PlayerWRs #added to be seen

CompleteData %>% #I don't remember why I was checking for this player, I think to verify he was a player and not an accidental input
...
```{r}
PlayerWRs2 <- CleanCompleteData2 %>% #again, updating to be its own dataframe because the results are more than 1000
  filter(!is.na(player) & player != "unknown player") %>%
  group_by(player, league) %>%
  summarize(WR = sum(result)/n(), Wins = sum(result), totalG = n()) %>%
  arrange(desc(Wins), desc(WR))

PlayerWRs2 #added to be seen
```
...
#Winrate with 30+ games in majors (established above)
CleanCompleteData2 %>%
...
#I'm also just now curious, 2 years later, to see it not arranged by wins and win rate but just grouped by each player so they can be seen side by side if they were ever in different leagues within this year. I have to remove the total games being more than 30 so that I can see their comparison to at the least MSI
CleanCompleteData2 %>%
  filter(!is.na(player) & player != "unknown player" & (league %in% Majors)) %>%
  group_by(player, league) %>%
  summarize(WR = sum(result)/n(), Wins = sum(result), totalG = n())
...
*LPL: Royal Never Give Up (RNG) 72% WR !!!Notably after cleaned data, EDward Gaming is now at a 72% WR instead of RNG
*MSI: Royal Never Give Up (RNG) 77% WR !!!And RNG remains at the top of MSI but with a 78% WR
...
FW3 #added to be seen

#Moving onto losses so I can compare
FastLoss <- CleanCompleteData2 %>%
  filter(gameLengthSeconds <= 1500 & is.na(player) & result == 0) %>%
  group_by(gameid) %>%
  select(gameid, team, league, game, side, gameLengthSeconds, result, teamkills, teamdeaths, firstblood, firstdragon, dragons, infernals, mountains, clouds, oceans, elders, firstherald, heralds, firstbaron, barons, firsttower, towers, firstmidtower, firsttothreetowers, inhibitors, dpm, damagetakenperminute, visionscore, monsterkillsenemyjungle, goldat10, goldat15, xpat10, xpat15, csat10, csat15, killsat10, killsat15, assistsat10, assistsat15, deathsat10, deathsat15)

FastLoss #added to be seen

FastLoss2 <- FastLoss %>%
  select(gameid, team, league, side, firstblood, firstdragon, firstherald, firsttower, result)

FastLoss2 #added to be seen
...
**Updates to this from a few years later, that line was likely written in haste. It is clear that the LPL is listed (a Major league) and for some reason the match with Game_id "ESPORTSTMNT04_1880348" is duplicated beyond belief! And it is duplicated in the CLEAN Data which means that there is likely even more cleaning that I needed to do. 
